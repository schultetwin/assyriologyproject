...
$searchbyvalue = TRIM($searchbyvalue);
...
/*Also we cannot assume that the value in a database row will be in some standard format so we format value, too. This is done with a user-defined mysql function, cleanedvalue, which I created.   This function does NOT change the entry in the database table, it merely changes the format that is being used in the SELECT statement below.  First, the function replaces each comma in value with a space.  Second, it removes all leading and following spaces.  Third, it adds a single space prefix and a single space suffice.  The concatenation used in the function is, of course, the mysql concatenation. */

$query = "SELECT  *  FROM nbxvectors WHERE cleanedvalue(value) COLLATE utf8mb4_0900_as_cs LIKE '%$searchbyvalue%' "; 
...
WHILE ($row = mysqli_fetch_assoc($result))                                     
	...
	echo "<br>";
	echo "CDLI number" . "  " . $row["cdlinum"] .  "  ";
	echo "Labat number" . "  " . $row["labatnum"] . "  ";
	echo "Borger number" . "  " . $row["borgernum"] . "  ";
	echo "<br>";
	echo $row["image_path"];
...
$array1 = $row;
/*The following results in $array1 keeping only those keys that are values in $nbtheorder.  */
$array1 = array_intersect_key($array1, array_flip($nbtheorder)); 
/*Next we insure that $array1 is in the proper order.      */  
$array1 = array_merge(array_flip($nbtheorder),  $array1);
