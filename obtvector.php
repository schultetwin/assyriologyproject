...
/* NEXT IS AN IMPORTANT PART OF THIS SCRIPT.  ONE ROW AT A TIME IS READ FROM THE TABLE obxvectors AND ITS DISTANCE FROM THE INPUT IS CALCULATED.  IF THIS DISTANCE IS LESS THAN OR EQUAL TO MAXDT, THIS ROW IS USED TO CONSTRUCT AN OUTPUT.*/
$query = "SELECT * FROM obxvectors";                                          
...
/*           */
	...
	echo "<br>";
	echo "The id of this candidate in the Old Babylonian table is   " . $output['xvectorvals']["id"];
	echo "<br>";
...
/*REMOVE EVERY ELEMENT IN $array2 WITH A KEY THAT IS NOT A VALUE IN $obtheorder.  REMEMBER $obtheorder IS AN INDEXED ARRAY.  THERE ARE JUST TWO: MAXDT AND submit.      */
$array2 = array_intersect_key($array2, array_flip($obtheorder));
/* array_flip interchanges keys and values. NOW the set of keys of $array2 is the same as the set of values of $obtheorder WE CAN NOW USE $obtheorder TO REORDER $array2 */
$array2 = array_merge(array_flip($obtheorder), $array2);
...
/*THE ARRAYS array_flip($properorderarray) AND $reducedarray1 HAVE EXACTLY THE SAME KEYS.  THE FUNCTION array_merge() IN THIS CASE CREATES AN ARRAY WITH, AGAIN, EXACTLY THE SAME KEYS WITH THE ORDER OF array_flip($properorderarray) AND THE VALUES OF  $reducedarray1: PRECISELY WHAT WE DESIRE.    */
	...
	echo "Michel number" . "  " . $output['xvectorvals']["michelnum"] . "  ";
	echo "<br>";
...
/* IF THE Modify SUBMIT BUTTON IS PUSHED, THE STRING $xencoded IS SENT TO OBeditingpage IN $_POST WHERE IT WILL BE base64_decode  and UNSERIALIZED. */
	
echo "
<form  method = 'POST' action = 'OBeditingpage.php' /> 
