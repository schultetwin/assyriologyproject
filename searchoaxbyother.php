...
/*Also we cannot assume that the value in a database row will be in some standard format so we format othervalues, too. This is done with a user-defined mysql function, cleanedother, which I created.   This function does NOT change the entry in the database table, it merely changes the format that is being used in the SELECT statement below.  First, the function replaces each comma and each = in othervalues with a space.  Second, it removes all leading and following spaces.  Third, it adds a single space prefix and a single space suffice.  The concatenation used in the function is, of course, the mysql concatenation. */

$query = "SELECT  *  FROM oaxvectors WHERE  cleanedother(othervalues) COLLATE utf8mb4_0900_as_cs  LIKE '%$searchbyother%' ";
