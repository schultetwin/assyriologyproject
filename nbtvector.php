...
/*           */
	...
	echo "<br>";
	echo "The id of this candidate in the Neo-Babylonian table is   " . $output['xvectorvals']["id"];
	echo "<br>";
...
/*Before we get started there may be a problem with $array2.  Its order may not be what we want, and we are going to order everything according to its order.  So first we are going to make $array2 have the order of the following indexed array.     */

$nbtheorder = array('DV', 'RH', 'HE', 'RHBB', 'DVBB', 'OBB', 'DVHT', 'RHHT', 'OHT', 'RDD', 'RUD', 'LDD', 'LUD', 'LH', 'UV', 'LHBB', 'UVBB', 'DNGR', 'VL', 'UDL', 'HL', 'DDL', '2C', 'L2T', 'R2T', 'T2T', 'B2T', 'NWC', 'NEC', 'SEC', 'SWC', 'O2T', '3T', '4T', 'H2T', 'V2T', 'RMC', 'TMC', 'BMC', 'LMC', 'UNKN');
/*REMOVE EVERY ELEMENT IN $array2 WITH A KEY THAT IS NOT A VALUE IN $nbtheorder.  REMEMBER $nbtheorder IS AN INDEXED ARRAY.  THERE ARE JUST TWO: MAXDT AND submit.      */
$array2 = array_intersect_key($array2, array_flip($nbtheorder));
/* array_flip interchanges keys and values. NOW the set of keys of $array2 is the same as the set of values of $nbtheorder WE CAN NOW USE $nbtheorder TO REORDER $array2 */
$array2 = array_merge(array_flip($nbtheorder), $array2);
...
/*THE ARRAYS array_flip($properorderarray) AND $reducedarray1 HAVE EXACTLY THE SAME KEYS.  THE FUNCTION array_merge() IN THIS CASE CREATES AN ARRAY WITH, AGAIN, EXACTLY THE SAME KEYS WITH THE ORDER OF array_flip($properorderarray) AND THE VALUES OF  $reducedarray1: PRECISELY WHAT WE DESIRE.    */
	...
	echo "<br>";
	echo "CDLI number" . "  " . $output['xvectorvals']["cdlinum"] . "  ";
	echo "Labat number" . "  " . $output['xvectorvals']["labatnum"] . "  ";
	echo "Borger number" . "  " . $output['xvectorvals']["borgernum"] . "  ";
	echo "<br>";
...
/* THE FOLLOWING LINES ARE CONCERNED WITH MODIFYING THE CORRESPONDING ROW IN THE IN THE DATABASE TABLE nbxvectors IF THE USER WANTS TO DO SO. */

/* WE WANT TO SEND $output['xvectorvals'] TO NBeditingpage, BUT  CANNOT SEND AN ARRAY SO WE FIRST SERIALIZE IT. BUT THAT IS NOT ENOUGH.  I NEED TO SEND IT AS A HIDDEN VARIABLE, AND SERIALIZED VARIABLES CANNOT BE HIDDEN SO I HAVE TO base64_encode IT.  $xencoded IS A STRING THAT I CAN SEND AS A HIDDEN VARIABLE. */
	$xencoded = base64_encode(serialize($output['xvectorvals']));
/* IF THE Modify SUBMIT BUTTON IS PUSHED, THE STRING $xencoded IS SENT TO NBeditingpage IN $_POST WHERE IT WILL BE base64_decode  and UNSERIALIZED. */
	
echo "
<form  method = 'POST' action = 'NBeditingpage.php' /> 

<input type='hidden' name = 'xencoded' id = 'xencoded'  value= '$xencoded' /> 

<input type = 'submit' name = 'modify1' id = 'modify1' value = 'Modify' />
</form>"; 
echo "<br>";
	$count++; echo "</div>";}
